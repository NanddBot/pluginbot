let handler = async (m, { conn, args }) => {
    ...
    if ((user.warn * 1) < 14) {
        ...
        m.reply('Kamu di warn oleh OWNER Atau MODERATOR!!, dan sekarang kamu punya *' + (user.warn + 1) + '* WARN. Ingat Jika kamu mendapat warn 15 kali kamu akan otomatis ke banned', mention)
    } else if ((user.warn * 1) > 13) {
        let reason = (args.length > 0 || args[1] ? args.slice(1).join(' ') : '15 kali WARN') || '15 kali WARN'
        user.Banneduser = true
        user.BannedReason = reason
        user.warn = 0
        m.reply('*Dia sudah Terbanned, karena mendapatkan 15 warn*')
        m.reply('*Kamu Terbanned karena telah mendapatkan 15 kali warn*\n *HUBUNGI* \n' + global.owner.map((v, i) => '*Owner ' + (i + 1) + ':* wa.me/' + v).join('\n') + '\n\n' + global.mods.map((v, i) => '*Moderator ' + (i + 1) + ':* wa.me/' + v).join('\n'), mention)
    }
}
