...
let handler = async (m, { conn, usedPrefix, owner }) => { 
     try {
        ...
        if (new Date - global.db.data.users[m.sender].lastfishing > 100000) {
...
${ usedPrefix }Cook catfish `    
        ...
        conn.reply( m.chat, mcng, m)
         ...
         conn.sendButton( m.chat, `You Get 🎁chest weapons epic ${psepick} item`, `NanddBot`, `again`, `.mancing`, m)
         } 
        if  (psenjata > 0 ) {
        	global.db.data.users[m.sender].psenjata+= psenjata * 1
        conn.sendButton( m.chat, `You Get 🎁chest weapons ${psenjata} item`, `NanddBot`, `Again`, `.mancing`, m)
        	}
        }, 38000)
...
handler.tags = ['rpg']
handler.command = /^(memancing|mancing)$/i 
