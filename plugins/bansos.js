...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        conn.sendFile( m.chat, kbansos, 'b.jpg', `Kamu Tertangkap Setelah Kamu korupsi dana bansos🕴️💰,  Dan kamu harus membayar denda 1 Juta rupiah💵`, m)
        user.money -= 1000000
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else if (Aku < Kamu) {
        user.money += 1000000
        conn.sendFile( m.chat, mbansos, 'b.jpg', `Kamu berhasil  korupsi dana bansos🕴️💰,  Dan kamu mendapatkan 1 Juta rupiah💵`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      } else {
