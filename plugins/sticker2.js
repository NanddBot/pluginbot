...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (stiker) await conn.sendMessage(m.chat, sticker, MessageType.sticker, {
        mimetype: 'image/wepg',
        contextInfo: {
        ...
        externalAdReply: {
        title: global.name,
        body: 'Nih stikernya beb..',
        previewType: 'PHOTO',
        thumbnail: `./src/logo.jpg`,
        sourceUrl: `wa.me` }}})

    else throw `Gagal${m.isGroup ? ', balas gambarnya!' : ''}`
  }
}
handler.help = ['stiker3']
handler.tags = ['sticker']
handler.command = /^s(tic?ker)?3?$/i
